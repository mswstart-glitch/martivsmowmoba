...
<style id="lumino-premium-transitions">
[data-lumino]{
...
<script id="lumino-premium-scroll-js">
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.section h2, .cta-box h2').forEach(title => {
        if(title.dataset.luminoSplit) return;
        title.dataset.luminoSplit = 'true';

        const words = title.textContent.trim().split(' ');
        title.innerHTML = words.map(w => `<span class="lumino-line"><span>${w}</span></span>`).join(' ');
    });

    gsap.utils.toArray('.section h2 .lumino-line span, .cta-box h2 .lumino-line span').forEach((line) => {
        gsap.from(line, {
            scrollTrigger:{
    ...
    gsap.utils.toArray('.grid-4, .packages, .results').forEach((group) => {
    ...
    gsap.utils.toArray('.card, .package, .result-card').forEach((el) => {
        gsap.to(el, {
            scrollTrigger:{
                trigger:el,
                start:'top bottom',
    ...
    gsap.utils.toArray('.cta-box').forEach((el) => {
        gsap.from(el, {
            scrollTrigger:{
                trigger:el,
                start:'top 82%',
